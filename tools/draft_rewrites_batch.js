...
function shorten(text) {
  // Conservative heuristics to shorten verbose choices while keeping meaning.
  if (!text) return text;
  let t = text.trim();
  // Remove parenthetical clauses
  t = t.replace(/\([^)]*\)/g, '').replace(/\s+/g,' ').trim();
  // If too long, split on semicolon or long conjunctions and keep first two parts
  const parts = t.split(/;|\. |, and |, which |, that |, so |, while |, and /i);
  if (parts.length > 2) {
    t = parts.slice(0,2).join(', ').trim();
  }
  // Limit length to ~140 chars by trimming trailing clauses
  if (t.length > 140) {
    // try cut at last comma before 140
...
while (true) {
  ...
  // backtrack to find the question text
  ...
  if (entries.length === 4) {
    ...
    // apply same flagging criteria as check script
...
// produce first batch of 20 rewrites
